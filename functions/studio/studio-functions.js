...
function reverseArray(arr){
    ...
    for (i = 0; i < arr.length; i++){
        newArray.unshift(reverseCharacters(arr[i]));
...
function funPhrase(str){
   let newPhrase = '';
    if(str.length <= 3){
        newPhrase = str.slice(-1, -1);
    } else if (str.length > 3){
        newPhrase = str.slice(0,3);
    }
    return `We put the ${newPhrase} in ${str}.`
}


// Test Function
let motto = "Functions rock!"
//console.log(funPhrase(motto));
